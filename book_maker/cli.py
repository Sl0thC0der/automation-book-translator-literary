...
""",
    ...
    parser.add_argument(
        ...
        dest="skip_review",
        action="store_true",
        help="(3pass mode) Skip pass 2 review, do translate only (faster/cheaper)",
    )
    parser.add_argument(
        "--translation-profile",
        dest="translation_profile",
        type=str,
        default=None,
    ...
    elif options.model.startswith("3pass"):
        API_KEY = options.claude_key or env.get("BBM_CLAUDE_API_KEY") or env.get("ANTHROPIC_API_KEY")
        if not API_KEY:
            raise Exception("Please provide Anthropic API key via --claude_key or ANTHROPIC_API_KEY env var")
    ...
    if options.model.startswith("3pass"):
        # Set the underlying Claude model
        if options.model == "3pass-opus":
            e.translate_model.set_claude_model("claude-opus-4-20250514")
        elif options.model == "3pass-sonnet":
            e.translate_model.set_claude_model("claude-sonnet-4-20250514")
        else:
            # default 3pass -> sonnet (good balance of quality/cost)
            e.translate_model.set_claude_model("claude-sonnet-4-20250514")
        e.translate_model.skip_review = options.skip_review

        # Load genre/style profile if specified
        if options.translation_profile:
            e.translate_model.load_profile(options.translation_profile)

        # Auto block_size for cost efficiency
        if options.block_size <= 0 and options.single_translate:
            e.block_size = 1500
            print(f"  Auto-set --block_size 1500 (override with --block_size N)")
        elif options.block_size <= 0 and not options.single_translate:
            print(f"  TIP: Use --single_translate for auto block_size (much cheaper)")

        # Force sequential — shared glossary/context state is not thread-safe
        if e.enable_parallel:
            print(f"  NOTE: --parallel-workers disabled for 3pass mode (stateful translator)")
            e.set_parallel_workers(1)

        model_name = e.translate_model.model
        mode = "2-pass (skip review)" if options.skip_review else "3-pass"
        profile = e.translate_model.profile_name
        block = f", block_size={e.block_size}" if e.block_size > 0 else ""
        print(f"  3-Pass Translation: model={model_name}, profile={profile}{block}, {mode}")
